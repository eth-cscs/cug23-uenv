...
\newcommand{\lstt}[1]{\lstinline{#1}\xspace}   % shorthand for inline listing
...
\begin{frame}[fragile]{}
    ...
    For those who don't -- this topic applies to all HPC clusters.
...
%-------------------------------------------
\begin{frame}[fragile]{Monolithic Software Stacks -- The Status Quo.}
    \begin{columns}[T]
    ...
    \begin{columns}[T]
        \begin{column}{0.6\textwidth}
            ...
            \begin{itemize}
                ...
                \item deployed by users and user-support -- \emph{without sys admin intervention};
...
> stack-config --recipe $recipe_path \
    --system $system_config_path --build /dev/shm/build
> cd /dev/shm/build
> env --ignore-environment PATH=/usr/bin:/bin:`pwd`/spack/bin make store.squashfs -j64
    ...
    \begin{itemize}
        ...
        \item \emph{system}: System-specific dependencies (gcc, libfabric, xpmem, slurm, rdma-core).
...
Name, mount point, and version of Spack to use.
...
%-------------------------------------------
\begin{frame}[fragile]{MPI Future Plans}
    Packaging \lstt{libpals}, \lstt{libpmi2} and \lstt{lib\_gtl\_\{cuda/hsa\}} together with \lstt{cray-mpich} doesn't scale
    \begin{itemize}
        \item vClusters with different versions of COS and Slurm require different combinations of each package.
        \item in Q1-2024 Alps will contain both \emph{aarch64} (Grace-Hopper) and \emph{x86\_64} nodes.
    \end{itemize}
    CSCS will soon package each component separately.

    \vspace{10pt}

    At the same time CSCS are collaborating with HPE to get access to nightly builds of these components to integrate into both CSCS' and HPE's CI/CD pipelines.
    \begin{itemize}
        \item Long term aim: integrate libfabric, and build components from source.
    \end{itemize}
    \emph{The engineer formalised the extraction process to a set of scripts that can be automated -- and CI will warn us early of braking changes.}
\end{frame}
%-------------------------------------------

%-------------------------------------------
\begin{frame}[fragile]{Cray-mpich at other sites}
    \begin{center}
    \emph{The packages are stored in an internal registry at CSCS.}

    \vspace{10pt}

    These are not accessible to other sites -- to use stackinator sites must perform the packaging locally.

    \vspace{10pt}

    We have to work with HPE to understand whether recent updates to their LICENSE to allow distributing container images with CPE artifacts covers this use case.
    \end{center}
...
\begin{frame}[fragile]{SLURM}
    ...
    \begin{code}{Launch with explicit flags}
            \begin{lstlisting}[style=talkbash]
% srun --uenv-file=img.squashfs -n2 -N2 osu_bw
            \end{lstlisting}
    \end{code}
    \begin{code}{Inherit the environment from the login node}
            \begin{lstlisting}[style=talkbash]
% uenv start img.squashfs
% srun -n2 -N2 osu_bw
            \end{lstlisting}
    \end{code}
...
\begin{frame}[fragile]{CI/CD}
    \begin{center}
    CI/CD pipelines \emph{from recipe to deployed SquashFS image} are used to deploy environments on vClusters from a centralised GitHub repository.
    \end{center}
    ...
    \begin{enumerate}
        ...
        \item push the generated image to an OCI registry (DockerHub API)
        \item \emph{\sc Test Stage}: pull the image and run a Slurm job that executes ReFrame tests.
        \item post status to GitHub
        \item \emph{\sc Deploy Stage}: \emph{manually} promote artifact to the deployment namespace.
    \end{enumerate}
...
\cscschapter{Status and Future Work}

%-------------------------------------------
\begin{frame}[fragile]{Status}
    \emph{Currently\dots}
    \begin{itemize}
        \item Stackinator worked ``out of the box'' on a Grace-Hopper white box.
        \item This approach is being used by the Swiss weather servce MeteoSchweiz on a vCluster without CPE installed.
        \item CSCS are providing uenv images to more users as they request software.
    \end{itemize}
    ...
    \emph{Presently\dots}
    \begin{itemize}
        \item CSCS will use uenv as the official method for deploying software on our Grace-Hopper roll out.
        \item The software will be refactored to make it easier for other sites to deploy.
        \item CSCS aims to provide alternatives to cray-mpich in 2024.
    \end{itemize}
\end{frame}
%-------------------------------------------
