...
% Performance data for the Sedov--Taylor test case on GPU nodes 
% reported by NVIDIA performance tool (Nsight Systems) for the Sedov--Taylor test case.

\begin{figure}[htp!]
    \begin{center}
        \input{images/sph-nvidia-nodes-scorep.tex}
    \end{center}
    \caption{Weak scaling profiling results on CPU nodes for SPH-EXA.}
    \label{fig:sph-weak-scorep}
\end{figure}

\fig{fig:sph-weak-scorep} shows profiling results for the Sedov--Taylor test case on CPU nodes.
The performance data was collected with the Score-P\footnote{\url{https://score-p.org}} tool.
