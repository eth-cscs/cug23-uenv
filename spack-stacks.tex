...
\begin{itemize}
    \item  \lstinline{config.yaml}:
        ...
        Used to specify where the image will be installed/mounted (the CSCS default is \emph{/user-environment}), optional configuration for a \spack build cache and the version of \spack. Reproducible builds use a fixed version/commit of \spack, and rolling releases use the \lst{develop} branch of \spack.
...
\end{lstlisting}
Two artifacts generated by the build:
\begin{itemize}
...
\assign{Simon: proof read and fix up, as you are more familiar with the process.}
